...
def db(filename: str,keyword:str):
    try:
        ...
        #cursor.execute("INSERT INTO itwork(work2) VALUES ('"+filename+keyword+"')");
        query = "update recipes_recipe \
                set \
                  recipe_image = '"+"recipe/"+filename+"'\
                WHERE\
                  title = '"+keyword+"'"
        #print (query,"Query")
...
def download(pool_sema: threading.Semaphore, url: str, output_dir: str,keyword:str):
    global in_progress
        ...
        try:
            ...
            for index, link in enumerate(links):
                if limit is not None and current + index >= limit:
                    return
                t = threading.Thread(target = download,args = (pool_sema, link, output_dir,keyword))
                t.start()
                current += 1
            last = links[-1]
        except IndexError:
